...
from models.aux_model.mamba.function_last import Cross_Modal_Mamba, PatchMerging

class IIMGF(nn.Module):
    def __init__(self, config, num_classes=5, embed_dims=[192, 384, 768, 768], input_resolution=[[28,28], [14,14], [7,7]],depth=[1,1,2,1],
        ...
        self.C2F_cross = nn.ModuleList([
            Cross_Modal_Mamba(d_model=embed_dims[0] * 2),
            Cross_Modal_Mamba(d_model=embed_dims[1] * 2),
        ])
        self.F2C = nn.ModuleList([])
        self.F2C_cross = nn.ModuleList([
            Cross_Modal_Mamba(d_model=embed_dims[1] * 2),
            Cross_Modal_Mamba(d_model=embed_dims[2] * 2),
        ])
