...
from models.IIMGF_two import IIMGF
...
def main(options):
    ...
    train_idx = options['train_idx']
    modal_path = options['modal_path']
    modal_path = os.path.join(modal_path, f'train_{train_idx}')
    ...
    try:
        ...
        if complete:
            ...
            with open("training_log.txt", "a") as file:
                # 将数字和字符串组合在一起并写入文件
                file.write(f"depth = {args.depth}, Avg_accuracy = {avg_acc:.4f}\n")
...
# dir_release = options['dir_release']
OPTIONS.add_argument('--dir_release',dest='dir_release',type=str,default="/home/wyl/work/data/derm7pt/release_v0")
# modal_path = options['modal_path']
...
OPTIONS.add_argument('--depth', dest='depth', type=int, nargs='+', default=[1, 1, 2, 1], help="List of depths")
OPTIONS.add_argument('--train_idx',dest='train_idx',type=int,default=1)
